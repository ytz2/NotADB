...
class ICallbackContainer {
 public:
  ...
  void registerListner(IListnerPtr cb) { appCallbacks_.push_back(cb); }
...
class ISession : public ICallbackContainer,
                 public std::enable_shared_from_this<ISession> {
 public:
  explicit ISession(const std::string &name) : name_(name), connected_(true) {}
...
class IAcceptor {
 public:
  ...
  bool removeSession(const std::string &name) {
    std::lock_guard<std::mutex> lk(lock_);
    if (!sessions_.count(name)) return false;
