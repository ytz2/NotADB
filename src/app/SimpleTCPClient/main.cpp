...
public: // IListener
	...
	virtual bool shouldProcess(const interface::IMessagePtr) override {
		return true;
	}
	virtual bool onMessage(interface::ISessionPtr session,
		...
		if (msg->ToString(body)) {
			LOG(INFO) << "Received message from connection "
					<< session->name() << ": " << body;
			return true;
		}
		return false;
...
public:
	SimpleClientListener(boost::asio::io_service &io) :
			timer_(std::make_shared<boost::asio::deadline_timer>(io)), session_(
					nullptr) {
	}
...
public:
	...
	virtual void onMessageSent(interface::ISessionPtr session,
	...
	void heartbeat(const boost::system::error_code &e) {
		...
		if (session_->connected()) {
			auto msg = std::make_shared<
					lib::message::commonjson::CommonJsonMessage>();
			auto &v = *msg;
			v["msg"] = "heartbeat";
			session_->send(msg);
		}
