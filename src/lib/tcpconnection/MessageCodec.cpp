...
BufferPtr FlatMessageDecoder::writeBuffer(interface::IMessagePtr msg) {
	...
	sprintf(header, "%4d%6d", msg->GetMessageID(), tmp.size());
	...
	return std::make_shared<boost::asio::mutable_buffer>((char*)e.c_str(), e.size());
}

bool FlatMessageDecoder::getMessages(
		std::vector<interface::IMessagePtr> &msgs) {
	std::size_t s;
	auto data = (char*)buffer_.getCurrentBuffer(s);
	while (s > flat_header_length) {
		try {
			int msgType = 0;
			int msgLen = 0;
			std::size_t size;
			msgType = std::stoi(std::string(data, 0, flat_msgType_length),
					&size, 10);
			msgLen = std::stoi(
					std::string(data, flat_msgType_length, flat_msgLen_length),
					&size, 10);
