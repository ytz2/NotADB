...
#ifndef SRC_LIB_TCPCONNECTION_MESSAGEBUFFER_H_
#define SRC_LIB_TCPCONNECTION_MESSAGEBUFFER_H_
#include <array>

// here we assume message size is smaller than 1MB, consider to change this one for working with large size msg
...
public:
	...
	const char* getCurrentBuffer(std::size_t &size) {
...
private:
	...
	const std::size_t safeLast_ = MESSAGE_BUFFER_SIZE - 1; // add one more trailer
};
 ...
 #endif /* SRC_LIB_TCPCONNECTION_MESSAGEBUFFER_H_ */
